# REIMAGINEDAPPV2 Architecture Documentation - Detailed
...
- **GET /api/chat/sessions**: Lists user chat sessions
  - [Calls: [chat.repository.js#listChatSessions()](#listchatsessions-function)]
  - [Middleware: validateResponse, EnvelopeSchema]
  - [Returns: Array of chat sessions with metadata]

- **GET /api/chat/history**: Retrieves chat message history for a thread
  - [Calls: [chat.repository.js#getChatHistory()](#getchathistory-function)]
  - [Parameters: threadId, limit, cursor]
  - [Returns: Paginated chat messages]

- **POST /api/chat/process**: Processes user messages and generates responses
  - [Calls: [chat-proxy.service.js#processUserMessage()](#processusermessage-function)]
  - [Body: userQuery, options]
  - [Returns: AI-generated response with metadata]

#### Document Routes
- **POST /api/document/ingest**: Ingests and processes documents
  - [Calls: [document.service.js#createIngestJob()](#createingestjob-function)]
  - [Body: file, metadata, options]
  - [Returns: Job ID and processing status]

- **GET /api/document/jobs/:jobId**: Gets document processing job status
  - [Calls: [document.repository.js#getJobStatus()](#getjobstatus-function)]
  - [Returns: Job status, progress, results]

#### Admin Routes
- **GET /admin/dashboard**: Returns admin dashboard overview
  - [Calls: [systems.repository.js#listSystems()](#listsystems-function)]
  - [Middleware: adminOnly, validateResponse]
  - [Returns: System metrics and status]

- **GET /admin/health**: System health check
  - [Calls: [guards/index.js#getExternalServiceStatus()](#getexternalservicestatus-function)]
  - [Returns: Service availability status]

### Business Logic Layer (42 services)

#### Chat Services
- **chat-proxy.service.js**: Main chat orchestration service
  - [processUserMessage()](#processusermessage-function): Processes user messages through Python sidecar
  - [retrieveWithSpecBias()](#retrievewithspecbias-function): Retrieves relevant context with specification bias
  - [listUserChats()](#listuserchats-function): Lists user's chat conversations

#### Document Services
- **document.service.js**: Document processing and management
  - [createIngestJob()](#createingestjob-function): Creates document ingestion jobs
  - [getDocumentStatus()](#getdocumentstatus-function): Gets document processing status
  - [extractTextPreview()](#extracttextpreview-function): Extracts text preview from documents

#### DIP Services
- **dip.service.js**: Document Intelligence Pipeline processing
  - [runDIPPacket()](#rundippacket-function): Runs DIP processing on documents
  - [checkDIPAvailability()](#checkdipavailability-function): Checks if DIP processing is available
  - [Calls External: Python Sidecar /v1/runDocIntelligencePacket]

#### LLM Services
- **llm.service.js**: Language model operations
  - [enhanceQuery()](#enhancequery-function): Enhances user queries for better retrieval
  - [summarizeConversation()](#summarizeconversation-function): Summarizes chat conversations
  - [generateChatName()](#generatechatname-function): Generates names for chat conversations
  - [synthesizeAnswer()](#synthesizeanswer-function): Synthesizes answers from retrieved context
  - [classifyQueryIntent()](#classifyqueryintent-function): Classifies user query intent
  - [generateAssetSummary()](#generateassetsummary-function): Generates summaries for assets

### Data Access Layer (14 repositories)

#### Chat Repository
- **chat.repository.js**: Chat data management
  - [listChatSessions()](#listchatsessions-function): Lists user chat sessions with pagination
  - [getChatHistory()](#getchathistory-function): Retrieves chat message history
  - [createChatMessage()](#createchatmessage-function): Creates new chat messages
  - [createChatMessageWithSequence()](#createchatmessagewithsequence-function): Creates messages with sequence numbers
  - [Tables: chat_sessions, chat_threads, chat_messages]

#### Document Repository
- **document.repository.js**: Document and job management
  - [updateJobStatusV2()](#updatejobstatusv2-function): Updates job status with version 2 schema
  - [updateJobDIPSuccess()](#updatejobdipsuccess-function): Updates DIP processing success status
  - [createOrUpdateDocument()](#createorupdatedocument-function): Creates or updates document records
  - [getJobStatus()](#getjobstatus-function): Retrieves job processing status
  - [Tables: jobs, documents]

#### Systems Repository
- **systems.repository.js**: System asset management
  - [listSystems()](#listsystems-function): Lists system assets with pagination
  - [getSystemByAssetUid()](#getsystembyassetuid-function): Retrieves system by asset UID
  - [Tables: systems, assets]

### Middleware Stack (9 middleware)
- **admin.js**: Admin-only access control
  - [adminOnly()](#adminonly-function): Validates admin authentication
  - [Checks: x-admin-token header]

- **validate.js**: Request validation
  - [validate()](#validate-function): Validates request body/query parameters
  - [Uses: Zod schemas]

- **validateResponse.js**: Response validation
  - [validateResponse()](#validateresponse-function): Validates API responses
  - [Uses: Zod schemas]

### Guards System
- **guards/index.js**: External service availability checks
  - [isPineconeConfigured()](#ispineconeconfigured-function): Checks Pinecone configuration
  - [isSupabaseConfigured()](#issupabaseconfigured-function): Checks Supabase configuration
  - [isOpenAIConfigured()](#isopenaiconfigured-function): Checks OpenAI configuration
  - [isSidecarConfigured()](#issidecarconfigured-function): Checks Python sidecar configuration
  - [getExternalServiceStatus()](#getexternalservicestatus-function): Gets status of all external services
...
### Document Processors
- **dip_processor.py**: Main document intelligence pipeline
  - [processDocument()](#processdocument-function): Processes documents through DIP pipeline
  - [extractStructuredData()](#extractstructureddata-function): Extracts structured data from documents
  - [Calls External: OpenAI GPT-4 API]
  - [Returns: Structured JSON, metadata, confidence scores]

### LLM Services
- **llm_service.py**: Language model interface
  - [generateResponse()](#generateresponse-function): Generates LLM responses
  - [createEmbeddings()](#createembeddings-function): Creates text embeddings
  - [Calls External: OpenAI /v1/chat/completions, /v1/embeddings]

### Utilities
- **file_utils.py**: File handling utilities
  - [validateFile()](#validatefile-function): Validates uploaded files
  - [extractText()](#extracttext-function): Extracts text from various file formats
  - [convertFormat()](#convertformat-function): Converts between file formats
...
**Called By**: [POST /api/chat/process](#post-apichatprocess)  
...
**Called By**: [GET /api/chat/sessions](#get-apichatsessions)
...
**Called By**: [GET /api/chat/history](#get-apichathistory)
...
**Calls**: [document.repository.js#createJob()](#createjob-function)  
**Called By**: [POST /api/document/ingest](#post-apidocumentingest)
...
**Called By**: [GET /admin/dashboard](#get-admindashboard)
...
**Called By**: [GET /admin/health](#get-adminhealth)
