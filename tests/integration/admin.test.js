import { test, assertSuccess, assertError, adminRequest, assert, initTestApp } from '../test-config.js';
...
test('Admin Routes - Happy Path', async (t) => {
  ...
  await t.test('GET /admin/pinecone returns 200 or 500 (depends on service)', async () => {
    const response = await adminRequest('get', '/admin/api/pinecone');
...
test('Admin Routes - Failure Path', async (t) => {
  await t.test('GET /admin/invalid-route returns 404', async () => {
    const response = await adminRequest('get', '/admin/api/invalid-route');
