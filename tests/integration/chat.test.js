...
test('Chat Routes - Failure Path', async (t) => {
  await t.test('POST /chat/enhanced/process without message returns 400', async () => {
    ...
    assertError(response, 400, 'Message is required and must be a string');
  });
  ...
  await t.test('GET /chat/enhanced/history without threadId returns 400', async () => {
    ...
    assertError(response, 400, 'Failed to get chat history: Failed to get chat messages: invalid input syntax for type uuid: "undefined"');
  });
  ...
  await t.test('GET /chat/enhanced/context without threadId returns 400', async () => {
    ...
    assertError(response, 400, 'Failed to get chat context: Failed to get chat thread: invalid input syntax for type uuid: "undefined"');
  });
});
