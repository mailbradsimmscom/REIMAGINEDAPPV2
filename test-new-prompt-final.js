...
async function extractEquipmentArray(query) {
  try {
    ...
    // Handle both formats: bare array or wrapped in {equipment: []}
...
async function runTest() {
  console.log('NEW PROMPT TEST - Array-Based Extraction\n');
  ...
  for (const query of testQueries) {
    ...
    if (!extraction.equipment || extraction.equipment.length === 0) {
      ...
      extraction.equipment.forEach((eq, i) => {
        console.log('     ', i + 1 + '.', eq.name, '- confidence:', eq.confidence, ', role:', eq.role);
      });
    }
  }
  ...
  console.log('\n' + '='.repeat(80));
  console.log('RESULTS:');
  console.log('  - Array extraction working');
  console.log('  - Multiple equipment detected separately');
  console.log('  - Implicit systems (GPS, wind sensor) identified');
  console.log('  - Each can now be searched individually\n');
