---
name: v5_chat_retrieval_v5_tags_and_doc_scoping
status: complete
...
owners:
  - brad
goal: >
  Align chat-time retrieval (“Pinecone out” + “DIP out”) with the v5 tagging model by
  replacing asset_uid-gated retrieval with model-tag filters (primary_models/is_universal, applies_to_models),
  and adding doc-candidate scoping (primary docs + referenced docs) for better relevance.
non_goals:
  - Changing the thread equipment_context blob schema (Option A: keep it stable)
  - Perfect per-record applicability classification; we aim for “very good relevance”
  - Migrating legacy Pinecone vectors or legacy DIP rows (start-fresh / best-effort compatibility)
  - Rewriting the Python sidecar chat workflow end-to-end (we will make targeted, localized edits only)
---
