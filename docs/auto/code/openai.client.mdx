...
description: Makes a structured JSON API call to OpenAI @param {Object} params - Configuration object @param {string} params.system - System prompt @param {string} params.user - User prompt @param {Object} params.schema - JSON schema for structured output @param {string} params.model - OpenAI model to use @param {number} params.maxOutputTokens - Maximum tokens to generate @param {number} params.seed - Seed for deterministic output @returns {Promise<Object>} - Parsed JSON response
---
