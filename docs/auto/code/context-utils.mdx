---
title: isFollowUpQuestion, containsAmbiguousPronoun, extractEquipmentTerms, hasExistingSystemsContext, getExistingSystemsContext, contextRewrite, withTimeout
description: Check if a query is a follow-up question that needs context @param {string} query - The user query @returns {boolean} - True if it's a follow-up question
---
