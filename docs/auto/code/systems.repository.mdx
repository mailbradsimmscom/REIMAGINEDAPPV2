---
title: Module
description: Lookup system metadata by normalized manufacturer and model names Used during document upload to resolve asset_uid and system metadata @param {string} manufacturerNorm - Normalized manufacturer name @param {string} modelNorm - Normalized model name @returns {Promise<Object>} System metadata with asset_uid, system_norm, subsystem_norm @throws {Error} If system not found or database error
---
